...
with tempfile.TemporaryDirectory(prefix='pkos-doc-zip-ui-') as tmp:
 ...
 try:
  result=subprocess.run(['node',str(app/'docs/점검도구/구형문서ZIP.mjs'),f'http://127.0.0.1:{server.server_port}/?localBridge=1',base64.b64encode(archive.getvalue()).decode()],cwd=app,timeout=220)
