...
function open(hooks){
 ...
 const recover=el('div');body.append(title,options,preview,status,transcript,controls,recover);
 ...
 head.append(button('닫기',()=>{if(state==='recording'||state==='paused'){finish();return;}if(state==='starting'||state==='saving')return;cleanup();panel.remove();active=null;}));panel.append(head,body);document.body.append(panel);
 ...
 async function begin(){
  ...
  state='starting';start.disabled=true;recover.hidden=true;
  try{
   ...
   const camera=await navigator.mediaDevices.getUserMedia({video:face.checked?{width:640,height:360}:false,audio:{echoCancellation:true,noiseSuppression:true}});streams.push(camera);
   db=await database();
   ...
   audio=new (window.AudioContext||window.webkitAudioContext)();await audio.resume();const destination=audio.createMediaStreamDestination();
   [screen,camera].forEach(stream=>{if(stream.getAudioTracks().length)audio.createMediaStreamSource(new MediaStream(stream.getAudioTracks())).connect(destination);});
   ...
   draw();drawTimer=setInterval(draw,33);const canvasStream=preview.captureStream(30);streams.push(canvasStream);const mixed=new MediaStream([...canvasStream.getVideoTracks(),...destination.stream.getAudioTracks()]);streams.push(destination.stream);
  ...
  }catch(e){cleanup();state='idle';start.disabled=false;recover.hidden=false;status.textContent='녹화를 시작하지 못했습니다: '+e.message;}
 }
 function guard(e){e.preventDefault();e.returnValue='';}
 function cleanup(){clearInterval(drawTimer);clearInterval(clockTimer);stopSpeech();streams.forEach(s=>s.getTracks().forEach(t=>t.stop()));streams=[];if(audio){audio.close().catch(()=>{});audio=null;}window.removeEventListener('beforeunload',guard);}
