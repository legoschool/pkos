...
self.addEventListener("fetch", function (e) {
  ...
  e.respondWith((async function () {
    ...
    try {
      /* navigate 요청에는 옵션을 붙여 새로 만들 수 없다 · 같은 주소를 캐시 확인 모드로 다시 묻는다 */
      var response = await fetch(e.request.url, { cache: "no-cache" });
      if (response.redirected) return fetch(e.request);
      if (response.ok) {
