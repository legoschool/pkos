...
try {
 ...
 check('지정한 실제 기록 폴더 연결',await evaluate(`pkosLocal.isOn()`));
 const count=await evaluate(`pkosLocal.entries().filter(n=>!n.trashed&&!n.localMissing).length`);
 check('실제 폴더의 기록 불러오기',count>0,String(count)+'개');
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS real folder connection; no test records created');
