...
try {
 await evaluate(`(async()=>{window.raceRoot=await(await navigator.storage.getDirectory()).getDirectoryHandle('attachment-race',{create:true});await pkosLocal.attach(raceRoot);const create=FileSystemFileHandle.prototype.createWritable;FileSystemFileHandle.prototype.createWritable=async function(...args){await new Promise(r=>setTimeout(r,350));return create.apply(this,args);};document.querySelector('.hm-quick input').value='즉시 저장 시험';document.querySelector('.hm-quick').requestSubmit();for(const text of ['FIRST','SECOND']){const dt=new DataTransfer();dt.items.add(new File([text],'same.txt',{type:'text/plain'}));document.querySelector('#blocks').dispatchEvent(new DragEvent('drop',{dataTransfer:dt,bubbles:true,cancelable:true}));}document.querySelector('#btnSave').click();})()`);
 await wait(3500);
 check('첨부 저장 직후 기록 저장 요청 보존',await evaluate(`pkosLocal.entries().some(n=>n.title==='즉시 저장 시험'&&n.blocks.filter(b=>b.fileId).length===2)`));
 check('연속 첨부의 경로와 바이트 분리',await evaluate(`(async()=>{const n=pkosLocal.entries().find(n=>n.title==='즉시 저장 시험');const bs=n.blocks.filter(b=>b.fileId);if(new Set(bs.map(b=>b.fileId)).size!==2)return false;let texts=[];for(const b of bs){let parts=b.fileId.slice(6).split('/'),h=raceRoot;for(const p of parts.slice(0,-1))h=await h.getDirectoryHandle(p);texts.push(await(await(await h.getFileHandle(parts.at(-1))).getFile()).text());}return texts.join(',')==='FIRST,SECOND';})()`));
 await send('Page.reload');await wait(1500);
 check('새로고침 후 첨부 경로 보존',await evaluate(`pkosLocal.entries().some(n=>n.title==='즉시 저장 시험'&&new Set(n.blocks.filter(b=>b.fileId).map(b=>b.fileId)).size===2)`));
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS attachment save race');
