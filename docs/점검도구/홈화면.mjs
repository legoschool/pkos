...
try {
 await evaluate(`localStorage.setItem('pkos.entries.v2',JSON.stringify(Array.from({length:5},(_,i)=>({id:'audit'+i,type:'experience',title:'긴 제목의 기록 '+i+' 공부와 수업 자료를 다시 확인하는 기록',tags:['수업','점검'],blocks:[{id:'b'+i,kind:'text',text:'최근 기록의 긴 본문이 화면 너비를 밀어내지 않는지 확인하는 문장입니다.'.repeat(4)}],createdAt:Date.now(),updatedAt:Date.now(),relations:[]}))))`);
 let failures=[];
 for(const width of [1440,768,390])for(const mode of ['light','dark'])for(const fontSize of ['','huge']){
  ...
  for(const page of ['home','library']){
   ...
   const v=await evaluate(`(()=>({overflow:document.documentElement.scrollWidth-innerWidth,mode:document.documentElement.dataset.mode,text:document.body.innerText}))()`);
   if(v.overflow>0)failures.push({width,mode,fontSize,page,overflow:v.overflow});
   if(/작은 다음 행동|생각이 머문 날/.test(v.text))throw Error('Removed panels present');
  }
 }
 console.log(JSON.stringify({cases:24,failures,errors}));
