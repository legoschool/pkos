...
try {
 const item=(id,title,dir,text)=>({id,title,localDir:dir,type:'material',tags:[],createdAt:1,updatedAt:1,relations:[],blocks:[{id:id+'b',kind:'text',text}]});
 const seed=[item('source','연결 출발','','[[동명]] [[constructor]] [[__proto__]]'),item('sameA','동명','폴더 A','A 내용'),item('sameB','동명','폴더 B','B 내용'),item('special1','constructor','','특수 제목 1'),item('special2','__proto__','','특수 제목 2')];
 ...
 await evaluate(`(()=>{document.querySelector('[data-page="library"].nav-item').click();const q=document.querySelector('#search');q.value='연결 출발';q.dispatchEvent(new Event('input',{bubbles:true}));document.querySelector('.card.entry .dots').click();Array.from(document.querySelectorAll('.menupop button')).find(b=>b.textContent.includes('전체 보기')).click();})()`);
 check('special and duplicate titles recognized',await evaluate(`document.querySelectorAll('.viewer .wikilink').length===3&&!document.querySelector('.viewer .wikilink.missing')`));
 await evaluate(`Array.from(document.querySelectorAll('.viewer .wikilink')).find(a=>a.textContent==='동명').click()`);
 check('duplicate title opens chooser',await evaluate(`document.querySelectorAll('.menupop button').length===2&&!!document.querySelector('.viewer')`));
 check('chooser distinguishes folders and bodies',await evaluate(`document.querySelector('.menupop').textContent.includes('폴더 A')&&document.querySelector('.menupop').textContent.includes('B 내용')`));
 check('chooser above reader',await evaluate(`Number(getComputedStyle(document.querySelector('.menupop')).zIndex)>Number(getComputedStyle(document.querySelector('.viewer')).zIndex)`));
 await evaluate(`Array.from(document.querySelectorAll('.menupop button')).find(b=>b.textContent.includes('폴더 B')).click()`);await wait(150);
 check('selected duplicate reached',await evaluate(`!!document.querySelector('[data-eid="sameB"].flash')&&!document.querySelector('[data-eid="sameA"].flash')`));
