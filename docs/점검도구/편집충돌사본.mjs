...
try {
 await evaluate(`(async()=>{window.conflictRoot=await(await navigator.storage.getDirectory()).getDirectoryHandle('edit-conflict',{create:true});let w=await(await conflictRoot.getFileHandle('original.md',{create:true})).createWritable();await w.write('# 원본 기록\\n\\n처음 본문');await w.close();await pkosLocal.attach(conflictRoot);document.querySelector('[data-page="home"].nav-item').click();const row=[...document.querySelectorAll('.hm-row')].find(n=>n.textContent.includes('원본 기록'));if(!row)throw Error('record not found');row.click();await new Promise(r=>setTimeout(r,300));document.querySelector('#blocks').innerHTML='<p>내가 작성한 글</p>';document.querySelector('#blocks').dispatchEvent(new InputEvent('input',{bubbles:true}));})()`);
 await wait(50);
 await evaluate(`(async()=>{const w=await(await conflictRoot.getFileHandle('original.md')).createWritable();await w.write('# 원본 기록\\n\\n외부에서 수정한 글');await w.close();document.querySelector('#btnSave').click();})()`);await wait(1200);
 check('외부 수정 원본 보존',await evaluate(`(async()=>{return(await(await(await conflictRoot.getFileHandle('original.md')).getFile()).text()).includes('외부에서 수정한 글');})()`));
 check('작성한 글을 독립된 충돌 사본에 저장',await evaluate(`pkosLocal.entries().some(n=>n.title.includes('(충돌 사본)')&&JSON.stringify(n.blocks).includes('내가 작성한 글')&&n.mdId!=='local:original.md'&&!n._dirty)`));
 await send('Page.reload');await wait(1200);
 check('재실행 후 충돌 사본 보존',await evaluate(`pkosLocal.entries().some(n=>n.title.includes('(충돌 사본)')&&JSON.stringify(n.blocks).includes('내가 작성한 글'))`));
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS edit conflict copy');
