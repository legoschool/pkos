...
try{
 if(process.argv.includes('--fail-store')) {
  await evaluate(`(()=>{const original=IDBObjectStore.prototype.put;window.injectedStorageFailure=false;IDBObjectStore.prototype.put=function(...args){if(this.name==='pending'&&!window.injectedStorageFailure){window.injectedStorageFailure=true;throw new DOMException('injected quota','QuotaExceededError');}return original.apply(this,args);};})()`);
 }
 await evaluate(`(()=>{document.querySelector('.hm-quick input').value='미업로드 첨부';document.querySelector('.hm-quick').requestSubmit();const dt=new DataTransfer();dt.items.add(new File([new Uint8Array([0,255,1,128,10,13,42])],'raw.bin',{type:'application/octet-stream'}));document.body.dispatchEvent(new DragEvent('drop',{dataTransfer:dt,bubbles:true,cancelable:true}));})()`);await wait(700);
 await evaluate(`document.querySelector('#btnSave').click()`);await wait(800);
 check('미업로드 첨부 저장',await evaluate(`pkosLocal.entries().some(n=>n.blocks.some(b=>b.kind==='file'&&!b.fileId&&b.blob))`));
 if(process.argv.includes('--fail-store'))check('첨부 저장 실패 주입 후 재시도',await evaluate(`window.injectedStorageFailure===true`));
 await send('Page.reload');await wait(1800);
 const bytes=await evaluate(`(async()=>{const b=pkosLocal.entries().flatMap(n=>n.blocks).find(b=>b.kind==='file');return b?.blob?Array.from(new Uint8Array(await b.blob.arrayBuffer())):null;})()`);
 check('새로고침 후 실제 IndexedDB 바이트 복원',JSON.stringify(bytes)==='[0,255,1,128,10,13,42]',JSON.stringify(bytes));
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS pending attachment browser persistence');
