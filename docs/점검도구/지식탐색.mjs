...
try {
 ...
 await evaluate(`Array.from(document.querySelectorAll('.modal .tab')).find(b=>b.textContent==='파일 이름').click()`);check('naming shows folder and title without date option',await evaluate(`document.querySelectorAll('input[name="recordNameRule"]').length===2&&!document.querySelector('.modal .mbody').textContent.includes('날짜')`));
 await evaluate(`document.querySelector('input[name="recordNameRule"][value="{폴더명}"]').click();Array.from(document.querySelectorAll('.modal .mfoot button')).find(b=>b.textContent==='저장').click();document.querySelector('[data-go="settings"]').click();Array.from(document.querySelectorAll('.modal .tab')).find(b=>b.textContent==='파일 이름').click()`);check('naming setting persists',await evaluate(`document.querySelector('input[name="recordNameRule"][value="{폴더명}"]').checked`));
