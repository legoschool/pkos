...
try{
 const result=await evaluate(`(async()=>{const f=await(await navigator.storage.getDirectory()).getDirectoryHandle('shared-trash',{create:true});async function put(n,t){const w=await(await f.getFileHandle(n,{create:true})).createWritable();await w.write(t);await w.close();}await put('shared.bin','PRESERVE-SHARED');await put('a.md','# A\\n\\n📎 [shared](shared.bin)');await put('b.md','# B\\n\\n📎 [shared](shared.bin)');await pkosLocal.attach(f);const a=pkosLocal.entries().find(n=>n.mdName==='a.md'),b=pkosLocal.entries().find(n=>n.mdName==='b.md');pkosLocal.trash(a.id);for(let i=0;i<50&&!a.blocks.some(b=>(b.fileId||" ").includes(".trash/"));i++)await new Promise(r=>setTimeout(r,50));const block=a.blocks.find(b=>b.kind==='file'),other=b.blocks.find(b=>b.kind==='file');if(block.fileId===other.fileId||other.fileId!=='local:shared.bin')throw Error('shared file moved');const original=await(await(await f.getFileHandle('shared.bin')).getFile()).text();await pkosLocal.purge(a.id);await new Promise(r=>setTimeout(r,500));if(await(await(await f.getFileHandle('shared.bin')).getFile()).text()!==original)throw Error('shared bytes removed');return original;})()`);
 check('공유 첨부를 휴지통 사본으로 분리하고 원본 보존',result==='PRESERVE-SHARED');
 check('다른 기록의 첨부 연결 유지',await evaluate(`pkosLocal.entries().find(n=>n.mdName==='b.md').blocks.some(b=>b.fileId==='local:shared.bin')`));
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS shared attachment trash protection');
