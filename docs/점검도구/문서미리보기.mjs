...
try {
 ...
 await evaluate(`(async()=>{const root=await navigator.storage.getDirectory();await pkosLocal.attach(root);const d=new DataTransfer();d.items.add(docFile('sample.docx'));window.dispatchEvent(new DragEvent('drop',{dataTransfer:d,bubbles:true,cancelable:true}));})()`);await wait(1500);
 ...
 await evaluate(`(()=>{const dt=new DataTransfer();dt.items.add(docFile('long.docx'));dt.items.add(docFile('long.xlsx'));dt.items.add(docFile('long.hwpx'));dt.items.add(docFile('long.pptx'));window.dispatchEvent(new DragEvent('drop',{dataTransfer:dt,bubbles:true,cancelable:true}));})()`);await wait(1200);
