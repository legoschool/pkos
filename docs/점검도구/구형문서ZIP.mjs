...
try {
 ...
 async function query(text){await evaluate(`(()=>{document.querySelector('[data-page="library"].nav-item').click();let input=document.getElementById('search');input.value=${JSON.stringify(text)};input.dispatchEvent(new Event('input'));})()`);await wait(500);}
 async function finishSearch(){for(let i=0;i<100;i++){if(await evaluate(`document.getElementById('attachmentSearchStatus').textContent.includes('완료')`))break;await wait(300);}}
 for (const ext of ['doc','xls','hwp']) {
  await query('sample.'+ext);
  await evaluate(`(()=>{const c=document.querySelector('.card.entry');if(!c)throw Error('record missing');c.querySelector('.dots').click();Array.from(document.querySelectorAll('.menupop button')).find(b=>b.textContent.includes('전체 보기')).click();})()`);await wait(250);
  ...
  if(ext==='xls') {
   ...
   await evaluate(`(()=>{const s=document.querySelector('.viewer select');s.value='1';s.dispatchEvent(new Event('change'));})()`);
   check('XLS second sheet and escaped text',await evaluate(`document.querySelector('.viewer').textContent.includes('SECOND XLS TOKEN')&&document.querySelector('.viewer').textContent.includes('<script>unsafe</script>')&&!document.querySelector('.viewer script')`));
  }
 ...
 await evaluate(`document.querySelector('[data-incoming-save]')?.click()`);
 for(let i=0;i<80;i++){if(await evaluate(`pkosLocal.entries().some(n=>n.blocks?.some(b=>b.fileId?.includes('bundle/nested/note.txt')))`))break;await wait(250);}
 check('Dropped ZIP extracted and imported',await evaluate(`pkosLocal.entries().some(n=>n.blocks?.some(b=>b.fileId?.includes('bundle/nested/note.txt')))`));
 check('Original ZIP retained',await evaluate(`fetch('/api/stat?path='+encodeURIComponent('받은 파일/bundle.zip')).then(r=>r.ok)`));
 await query('ZIP BODY TOKEN');await finishSearch();check('Extracted body searchable',await evaluate(`document.querySelectorAll('.card.entry').length===1`));
 await send('Page.reload');await wait(2200);await query('LASTROW XLS TOKEN');await finishSearch();check('Search after reconnect',await evaluate(`document.querySelectorAll('.card.entry').length===1`));
