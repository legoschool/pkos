...
try {
 ...
 await evaluate(`document.querySelector('[data-page="library"].nav-item').click()`);await wait(500);
 await evaluate(`(()=>{const c=document.querySelector('.card.entry');c.querySelector('.dots').click();Array.from(document.querySelectorAll('.menupop button')).find(b=>b.textContent.includes('전체 보기')).click();})()`);await wait(300);
 check('attachment card',await evaluate(`!!document.querySelector('.viewer .attachment-card')`));
 await evaluate(`document.querySelector('.viewer .attachment-menu summary').click()`);
 check('open menu actions',await evaluate(`(()=>{const t=document.querySelector('.viewer .attachment-options').textContent;return ['기본 앱에서 열기','파일 탐색기에서 보기','사본 다운로드'].every(v=>t.includes(v));})()`));
