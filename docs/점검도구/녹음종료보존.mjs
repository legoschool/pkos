...
try{
 await evaluate(`(async()=>{window.realMic=navigator.mediaDevices.getUserMedia.bind(navigator.mediaDevices);window.lateStream=await realMic({audio:true});navigator.mediaDevices.getUserMedia=()=>new Promise(r=>window.resolveMic=r);window.SpeechRecognition=undefined;window.webkitSpeechRecognition=undefined;document.querySelector('.hm-quick input').value='늦은 마이크';document.querySelector('.hm-quick').requestSubmit();document.querySelector('[data-add="voice"]').click();Array.from(document.querySelectorAll('.modal button')).find(b=>b.textContent.includes('녹음 시작')).click();document.querySelector('.modal .mhead button').click();resolveMic(lateStream);})()`);await wait(500);
 check('닫은 뒤 도착한 마이크 스트림 종료',await evaluate(`lateStream.getTracks().every(t=>t.readyState==='ended')&&!document.querySelector('.modal')`));
 await evaluate(`(()=>{navigator.mediaDevices.getUserMedia=realMic;document.querySelector('[data-add="voice"]').click();Array.from(document.querySelectorAll('.modal button')).find(b=>b.textContent.includes('녹음 시작')).click();})()`);await wait(800);
 await evaluate(`document.querySelector('.modal .mhead button').click()`);await wait(1200);
 check('녹음 중 닫기도 음성 기록 보존',await evaluate(`pkosLocal.entries().some(n=>n.blocks.some(b=>b.kind==='file'&&b.blob&&b.blob.size>0))&&!!document.querySelector('.viewer')`));
 if(errors.length||results.some(r=>!r.ok))throw Error(errors.join('\n')||'check failed');console.log('PASS voice lifecycle');
