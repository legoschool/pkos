...
try{
 ...
 await evaluate(`(()=>{const row=[...document.querySelectorAll('.tagrow')].find(n=>n.textContent.includes('old'));row.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:100,clientY:300}));})()`);
 check('태그 우클릭 앱 메뉴',await evaluate(`document.body.textContent.includes('태그 이름 바꾸기')`));
 await evaluate(`(()=>{const nodes=[...document.querySelectorAll('.menupop *')];const button=nodes.find(n=>n.textContent==='태그 이름 바꾸기');if(!button)throw Error('menu not found');button.click();document.querySelector('[aria-label="새 이름"]').value='renamed';document.querySelector('[data-rename-save]').click();})()`);
 await wait(1800);
 check('태그 실제 폴더 반영',await evaluate(`fetch('/api/stat?path=renamed').then(r=>r.ok)`));
 check('태그 목록 반영',await evaluate(`pkosLocal.entries().some(n=>n.tags.includes('renamed'))&&!pkosLocal.entries().some(n=>n.tags.includes('old'))`));
 check('원래 폴더 사라짐',await evaluate(`fetch('/api/stat?path=old').then(r=>r.status===404)`));
 await evaluate(`(()=>{const row=[...document.querySelectorAll('.siderow[data-drop-path]')].find(n=>n.dataset.dropPath==='["renamed"]');if(!row)throw Error('folder row not found');row.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX:100,clientY:300}));const button=[...document.querySelectorAll('.menupop *')].find(n=>n.textContent==='이름 바꾸기');button.click();document.querySelector('[aria-label="새 이름"]').value='final';document.querySelector('[data-rename-save]').click();})()`);await wait(1600);
 check('폴더 우클릭 실제 반영',await evaluate(`fetch('/api/stat?path=final').then(r=>r.ok)`));
 await send('Page.reload');await wait(1800);
 check('재실행 후 경로 유지',await evaluate(`pkosLocal.entries().some(n=>n.mdId==='local:final/note.md'&&n.tags.includes('renamed'))`));
